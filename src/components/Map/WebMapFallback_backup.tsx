...
const WebMapFallback = memo(function WebMapFallback({ onStationPress }: WebMapFallbackProps) {
  ...
  const handleToggleFavorite = useCallback(async (station: Station, event: any) => {
    ...
    try {
      ...
      console.log('[WebMapFallback] 現在のお気に入り数:', storageState.favoriteStations.length);
      console.log('[WebMapFallback] 現在のお気に入りリスト:', storageState.favoriteStations.map(s => s.name));

      const isFavoriteNow = storageState.favoriteStations.some(fav => fav.id === station.id);
      console.log('[WebMapFallback] 現在のお気に入り状態:', isFavoriteNow);

      console.log('[WebMapFallback] storageActions.toggleFavorite実行中...');
      ...
      console.log('[WebMapFallback] UI強制更新:', newRefreshKey);
      setRefreshKey(newRefreshKey);

      // 状態再確認
      setTimeout(() => {
        console.log('[WebMapFallback] 切り替え後のお気に入り数:', storageState.favoriteStations.length);
        console.log('[WebMapFallback] 切り替え後のお気に入りリスト:', storageState.favoriteStations.map(s => s.name));
      }, 500);
  ...
  const renderStationInfo = useCallback((station: Station, index: number) => {
    ...
    if (index < 3) {
      console.log(`[WebMapFallback] 駅表示: ${station.name} (${station.id}), お気に入り: ${isFavorite}, 全お気に入り数: ${storageState.favoriteStations.length}`);
    }
  ...
  }, [storageState.favoriteStations, onStationPress, handleToggleFavorite, refreshKey]);
      ...
      >
        ...
        <View style={styles.categoryContainer}>
          ...
          >
            <Text style={getCategoryTextStyle('favorites')}>
              お気に入り ({storageState.favoriteStations.length})
            </Text>
          </TouchableOpacity>
        </View>
        ...
        <View style={styles.statsContainer}>
          <Text style={styles.statsText}>
            ...
            お気に入り: {storageState.favoriteStations.length}駅 |
